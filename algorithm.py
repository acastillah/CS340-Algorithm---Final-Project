...
def fill_classroom(classroomSize, availableStudents, timeslot, ds):
    studentsInClass = []
...
def registrars(ds):
    ...
    for timeslot in ds["Timeslots"]:
        ...
        pair = fill_classroom(classroomSize, ds["PossibleStudents"][course], timeslot, ds)
        ds = pair[1]
    ...
    while ds["PopularClasses"] and ds["PossibleClassrooms"]:
        ...
        pair = fill_classroom(classroomSize, availableStudents, optimalTimeslot, ds)
        ds = pair[1]
        studentsInClass = pair[0]
        teacher = ds["ClassTeacher"][course]
